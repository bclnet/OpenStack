...
namespace OpenStack.Graphics.OpenGL
{
    ...
    public class GLMeshBuffers
    {
        ...
        public GLMeshBuffers(IVBIB vbib)
        {
            ...
            for (var i = 0; i < vbib.VertexBuffers.Count; i++)
            {
                ...
                GL.GetBufferParameteri64(BufferTarget.ArrayBuffer, BufferPName.BufferSize, ref VertexBuffers[i].Size);
            }
            for (var i = 0; i < vbib.IndexBuffers.Count; i++)
            {
                ...
                GL.GetBufferParameteri64(BufferTarget.ElementArrayBuffer, BufferPName.BufferSize, ref IndexBuffers[i].Size);
            }
        }
    }
    ...
    public class GLMeshBufferCache
    {
        ...
        public uint GetVertexArrayObject(IVBIB vbib, Shader shader, uint vtxIndex, uint idxIndex, uint baseVertex)
        {
            ...
            // build
            var newVaoHandle = 0; GL.GenVertexArrays(1, ref newVaoHandle);
        ...
        {
            var location = (uint)GL.GetAttribLocation(shaderProgram, attributeName);
            if (location == uint.MaxValue) return; // Ignore this attribute if it is not found in the shader
            GL.EnableVertexAttribArray(location);
            switch (attribute.Format)
            {
                case DXGI_FORMAT.R32G32B32_FLOAT: GL.VertexAttribPointer(location, 3, VertexAttribPointerType.Float, false, stride, (IntPtr)(baseVertex + attribute.Offset)); break;
                case DXGI_FORMAT.R8G8B8A8_UNORM: GL.VertexAttribPointer(location, 4, VertexAttribPointerType.UnsignedByte, false, stride, (IntPtr)(baseVertex + attribute.Offset)); break;
                case DXGI_FORMAT.R32G32_FLOAT: GL.VertexAttribPointer(location, 2, VertexAttribPointerType.Float, false, stride, (IntPtr)(baseVertex + attribute.Offset)); break;
                case DXGI_FORMAT.R16G16_FLOAT: GL.VertexAttribPointer(location, 2, VertexAttribPointerType.HalfFloat, false, stride, (IntPtr)(baseVertex + attribute.Offset)); break;
                case DXGI_FORMAT.R32G32B32A32_FLOAT: GL.VertexAttribPointer(location, 4, VertexAttribPointerType.Float, false, stride, (IntPtr)(baseVertex + attribute.Offset)); break;
                case DXGI_FORMAT.R8G8B8A8_UINT: GL.VertexAttribPointer(location, 4, VertexAttribPointerType.UnsignedByte, false, stride, (IntPtr)(baseVertex + attribute.Offset)); break;
                case DXGI_FORMAT.R16G16_SINT: GL.VertexAttribIPointer(location, 2, VertexAttribIType.Short, stride, (IntPtr)(baseVertex + attribute.Offset)); break;
                case DXGI_FORMAT.R16G16B16A16_SINT: GL.VertexAttribIPointer(location, 4, VertexAttribIType.Short, stride, (IntPtr)(baseVertex + attribute.Offset)); break;
                case DXGI_FORMAT.R16G16_SNORM: GL.VertexAttribPointer(location, 2, VertexAttribPointerType.Short, true, stride, (IntPtr)(baseVertex + attribute.Offset)); break;
                case DXGI_FORMAT.R16G16_UNORM: GL.VertexAttribPointer(location, 2, VertexAttribPointerType.UnsignedShort, true, stride, (IntPtr)(baseVertex + attribute.Offset)); break;
    ...
    public class GLRenderMaterial : RenderMaterial
    {
        ...
        public override void Render(Shader shader)
        {
            ...
            foreach (var texture in Textures)
            {
                ...
                {
                    GL.ActiveTexture((int)TextureUnit.Texture0 + (TextureUnit)textureUnit);
            ...
            switch (Material)
            {
                case IParamMaterial p:
                    foreach (var param in p.FloatParams)
                    {
                        ...
                        if (uniformLocation > -1) GL.Uniform1f(uniformLocation, param.Value);
                    }
                    foreach (var param in p.VectorParams)
                    {
                        uniformLocation = shader.GetUniformLocation(param.Key);
                        if (uniformLocation > -1) GL.Uniform4f(uniformLocation, param.Value.X, param.Value.Y, param.Value.Z, param.Value.W);
                    }
                    break;
            }
