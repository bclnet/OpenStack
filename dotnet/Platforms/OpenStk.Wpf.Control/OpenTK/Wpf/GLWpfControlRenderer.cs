...
internal sealed class GLWpfControlRenderer : IDisposable {
    ...
    public void ReallocateFramebufferIfNeeded(double width, double height, double dpiScaleX, double dpiScaleY, Format format, int samples) {
        var newWidth = (int)Math.Ceiling(width * dpiScaleX);
        var newHeight = (int)Math.Ceiling(height * dpiScaleY);
        ...
        if (D3dImage == null || FramebufferWidth != newWidth || FramebufferHeight != newHeight || Samples != samples) {
            ...
            if (width > 0 && height > 0) {
                ...
                var dxColorRenderTargetShareHandle = IntPtr.Zero;
                ...
                var success = Wgl.NV.DXSetResourceShareHandleNV(DxColorRenderTarget.Handle, dxColorRenderTargetShareHandle);
                if (success == false) {
                ...
                var prevFramebuffer = GL.GetInteger(GetPName.DrawFramebufferBinding);
                var prevRenderbuffer = GL.GetInteger(GetPName.RenderbufferBinding);
                ...
                DxInteropColorRenderTargetRegisteredHandle = Wgl.NV.DXRegisterObjectNV(
                    ...
                    (ObjectTypeDX)TextureTarget.Texture2D,
                ...
                if (Samples > 1) {
                    GL.RenderbufferStorageMultisample(RenderbufferTarget.Renderbuffer, Samples, InternalFormat.Depth24Stencil8, FramebufferWidth, FramebufferHeight);
                }
                else {
                    GL.RenderbufferStorage(RenderbufferTarget.Renderbuffer, InternalFormat.Depth24Stencil8, FramebufferWidth, FramebufferHeight);
                }
    ...
    public void Render(DrawingContext drawingContext) {
        ...
        var success = Wgl.NV.DXLockObjectsNV(_context.GLDeviceHandle, 1, new[] { DxInteropColorRenderTargetRegisteredHandle });
        if (success == false) {
        ...
        success = Wgl.NV.DXUnlockObjectsNV(_context.GLDeviceHandle, 1, new[] { DxInteropColorRenderTargetRegisteredHandle });
        if (success == false) {
        ...
        var rect = new System.Windows.Rect(0, 0, D3dImage.Width, D3dImage.Height);
