...
class BasicXEffect : Effect {
    ...
    static byte[] GetShader() { using var ms = typeof(BasicXEffect).Assembly.GetManifestResourceStream("OpenStk.Platform.Mg.Name.shaders.IsometricWorld.fxc"); var b = new byte[ms.Length]; ms.ReadExactly(b); return b; }
}
...
public class XbrEffect : Effect {
    ...
    static byte[] GetShader() { using var ms = typeof(BasicXEffect).Assembly.GetManifestResourceStream("OpenStk.Platform.Mg.Name.shaders.xBR.fxc"); var b = new byte[ms.Length]; ms.ReadExactly(b); return b; }
}
