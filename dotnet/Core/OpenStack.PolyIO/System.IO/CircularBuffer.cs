...
/// </summary>
public class CircularBuffer(int size = 4096) {
    ...
    void SetCapacity(int capacity) {
        ...
        if (Length > 0) {
            if (_head < _tail) _buffer.AsSpan(_head, Length).CopyTo(newBuffer.AsSpan());
            else { _buffer.AsSpan(_head, _buffer.Length - _head).CopyTo(newBuffer.AsSpan()); _buffer.AsSpan(0, _tail).CopyTo(newBuffer.AsSpan(_buffer.Length - _head)); }
        }
        _head = 0;
        _tail = Length;
        _buffer = newBuffer;
    ...
    public void Enqueue(Span<byte> buffer, int offset, int size) {
        ...
        if (_head < _tail) {
            ...
            if (rightLength >= size) buffer.Slice(offset, size).CopyTo(_buffer.AsSpan(_tail));
            else { buffer.Slice(offset, rightLength).CopyTo(_buffer.AsSpan(_tail)); buffer.Slice(offset + rightLength, size - rightLength).CopyTo(_buffer.AsSpan()); }
        }
        else buffer.Slice(offset, size).CopyTo(_buffer.AsSpan(_tail));
    ...
    public int Dequeue(Span<byte> buffer, int offset, int size) {
        ...
        if (size == 0) return 0;
        if (_head < _tail) _buffer.AsSpan(_head, size).CopyTo(buffer[offset..]);
        else {
            var rightLength = _buffer.Length - _head;
            if (rightLength >= size) _buffer.AsSpan(_head, size).CopyTo(buffer.Slice(offset));
            else { _buffer.AsSpan(_head, rightLength).CopyTo(buffer[offset..]); _buffer.AsSpan(0, size - rightLength).CopyTo(buffer[(offset + rightLength)..]); }
        }
        _head = (_head + size) % _buffer.Length;
        Length -= size;
        if (Length == 0) { _head = 0; _tail = 0; }
        return size;
    }

    public int DequeSegment(int size, out ArraySegment<byte> segment) {
        if (size > Length) size = Length;
        if (size == 0) { segment = new ArraySegment<byte>(); return 0; }
        if (_head >= _tail) size = Math.Min(size, _buffer.Length - _head);
